...
import random
import time
#allows for pixel manipulation and pixel properties
...
SCREEN_WIDTH = 1200
SCREEN_HEIGHT = 800
screen_title = "Shortest path calculator"
...
def set_icon():
        #create the icon + resizing
        icon = Image.open("images\\main_window_icon.ico") #assigns an icon variable to the icon 
        resized_icon = icon.resize((80,80)) #sets the icons size perameter

        # Convert the resized image to PNG
        png_icon = Image.new("RGBA", resized_icon.size) #converts the icon to the png 
        png_icon.paste(resized_icon, (0, 0), mask=resized_icon) #pastes icon onto the image

        #png_icon.tobytes() converts the image to a string of RGB values, then pygame transforms
        #this into a pygame compatible image using the string
        return pygame.image.fromstring(png_icon.tobytes(), png_icon.size, "RGBA")

def drawGraph(graph,shortest_path,circle_colour,image, costs,stats):
    ...
    for node in reversed(graph): #itterates through each node in the graph 
        ...
        if costs != {} and stats == True:
            if node[0][0] in costs.keys():
                if costs[node[0][0]] != 999999999999 and costs[node[0][0]] > 0.5:
                    draw_text(str(round(costs[node[0][0]])), small_text_font, "red", (node[0][1][0],node[0][1][1]-node[0][2]-10))
...
def drawLinkLine(start_coords,end_coords):
    pygame.draw.line(screen,"black",start_coords,end_coords,1)
...
def distance_to_edge(x, y, start_x, start_y, end_x, end_y):
    ...
    #the line above calculates the decimal percentage of how far along the closest point position to the mouse click is 
        ...
        print ("error line 149: tryna div by zero")
...
def randomiser(randomiser_num): #takes the number of nodes to randomise as a parameter
        ...
        if randomiser_num >= 16: #if there are more than 17 nodes then\/
            radius1 = 30 #the circle size is partially reduced
        else: #if there is less than 17 nodes
            radius1 = 50 #the radius will stay the normal size

        #creating nodes
        #this for loop iterates for every node in the randomiser_num variable
        for i in range(0,randomiser_num):
            xy = (random.randint(81,1124),random.randint(72,573))
            if i == 1:
...
class Button():
    def __init__(self,pygame_screen ,text,colour,transparency,x,y,size): #constructor
        self.text_font = pygame.font.SysFont("comic sans",30)
        self.text = text
        ...
        self.img = self.text_font.render(text,True,(0,0,0)) #renders text to be put onto button
    ...
    def center_text(self,text,coordinates,button):
        if button == True:
            if len(text) == 1:
                return (coordinates[0]-6.5,coordinates[1]-20)
            centered_offset_x = 0 #declares the variable
            for i in range(0,len(text)-1): #this itterates through every character in the text
                centered_offset_x = centered_offset_x + 8.65 #adds to the coordinates for every letter in the text on the button
            centered_coordinates = (coordinates[0]-centered_offset_x,coordinates[1]-20) #edits the coordinates of the actual text 
    ...
    def coordinates_on_button(self,pos):
        if pos[0] >= self.xy[0] and pos[0] <= self.xy[0]+self.size[0]: #checks if the X coordinates are in range 
            if pos[1] >= self.xy[1] and pos[1] <= self.xy[1]+self.size[1]: #checks if the Y coordinates are in range 
                return True
        return False
    
    def draw(self,border,mouse_position):   
        if border == False:
            if self.coordinates_on_button(mouse_position) == True:
                self.perameters.set_alpha(75)
            else:
                self.perameters.set_alpha(self.transparency)
            self.screen.blit(self.perameters, self.xy)  #draws button to screen
            self.screen.blit(self.img,self.center_text(self.text,self.middle_xy,True)) #draws text on the button
        ...
        if border == True:
            ...
            if self.coordinates_on_button(mouse_position) == True:
                border_box.set_alpha(50)
            else:
                border_box.set_alpha(256)
            self.screen.blit(border_box, (self.xy[0]-5,self.xy[1]-5))  #draws button to screen
            
            self.perameters.set_alpha(256) #sets transparency
            self.screen.blit(self.perameters, self.xy)  #draws button to screen
            self.screen.blit(self.img,self.center_text(self.text,self.middle_xy,True)) #draws text on the button

    def draw_icon(self,address):
        #create the icon + resizing
        icon = Image.open(address) #assigns an icon variable to the icon 
        resized_icon = icon.resize(self.size) #sets the icons size perameter
        ...
        #png_icon.tobytes() converts the image to a string of RGB values, then pygame transforms
        #this into a pygame compatible image using the string
        surface = pygame.image.fromstring(png_icon.tobytes(), png_icon.size, "RGBA")

        #the image is drawn to the surface at the coordinates
        self.screen.blit(surface, (self.xy)) 
    ...
    def return_button_info(self):
        return ((self.coordinates(),self.getsize(),self.getName()))
...
class Shortestpath():
    ...
    def getDistances(self):
        ...
        for node in self.graph: #itterates through each node in the graph
    ...
    def average_distance(self,dictionary):
    ...
    def average_weight(self):
        total = 0 #declares a total variable
        length = 0 #declares a length variable
        not_zero = False #declares the variable that avoids the div by 0 error
        for node in self.graph: #itterates through graph
            #itterates through each element in the dictionary
            for weight in node[2].values(): 
                total = weight + total #sums the total value
                length = length + 1 #sums the length
                not_zero = True #of there is a link then this is set to True
        if not_zero == True: #If there is more than 0 links
            average = total/length #calculates the total value
        else: #this will run if there are 0 connections in the graph
            average = 1 #sets the average to 1 to avoid the program dividing by 0
        return average #returns total value 
    
    def aStar(self):
        #below is the code that was in the a* function 
        ...
        #holds the distance of each node to the destinaiton 
        distances_dict = self.getDistances()
        #calculates the scale factor by dividing the the average weights and distances
        scale = self.average_weight()/self.average_distance(distances_dict)
        
        for node in self.graph: #cycles through each node in the graph 
        ...
        while True:
            ...
            for connection in self.graph[IndexFromNodeID(self.graph, lowest_node)][1]: 
                if connection not in nodes_explored: #this will be True if the connection hasn't been explored
                    ...
                    weight = getWeight(self.graph[IndexFromNodeID(self.graph, lowest_node)], self.graph[IndexFromNodeID(self.graph, connection)])
        ...
        return (shortest_path,node_estimated_cost_dict[destination],node_estimated_cost_dict)        
    ...
    def dijkstra(self):
        ...
        for node in processed_graph: #cycles through each node in the graph 
            if node[0][5] == "destination": #checks if a node is the destination node 
                destination = node[0][0] #assigns a destination node
            if node[0][5] == "starting":#checks if a node is the starting node 
                node_estimated_cost_dict.update({node[0][0]:0}) #updates the nodes estimated cost as 0
        ...
        while True:
            ...
            #looks at connection of the exploring nodes connection array
            for connection in processed_graph[IndexFromNodeID(processed_graph, lowest_node)][1]: 
                if connection not in nodes_explored: #this will be True if the connection hasn't been explored
                    #calculates the weights of the connection node and linking nodes
                    weight = getWeight(processed_graph[IndexFromNodeID(processed_graph, lowest_node)], processed_graph[IndexFromNodeID(processed_graph, connection)])
...
#making buttons
button_move = Button(screen,"Move",(5, 200, 235),200,608,650,defult_size) #creating an object from the button class
buttons.append((button_move.coordinates(),button_move.getsize())) #adding the first button to the button array
button_delete = Button(screen,"Delete",(255,0,127),134,905,650,defult_size) #creating an object from the button class
buttons.append((button_delete.coordinates(),button_delete.getsize())) #adding the second button to the button array
button_create = Button(screen,"Create",(16, 222, 22),134,10,650,defult_size) #creating an object from the button class
buttons.append((button_create.coordinates(),button_create.getsize())) #adding the third button to the button array
button_link = Button(screen,"Link",(242, 242, 5),134,307,650,defult_size) #creating an object from the button class
buttons.append((button_link.coordinates(),button_link.getsize())) #adding the fourth button to the button array

button_edit = Button(screen,"Edit",(255,128,0),200,100,20,(60,60)) #creating an object from the button class
buttons.append((button_edit.coordinates(),button_edit.getsize())) #adding the fifth button to the button array
button_run = Button(screen,"Run",(133, 224, 255),200,180,20,(60,60)) #creating an object from the button class
buttons.append((button_run.coordinates(),button_run.getsize())) #adding the sixth button to the button array
button_settings = Button(screen,"Settings",(177,177,177),100,20,20,(60,60)) #creating an object from the button class    
buttons.append((button_settings.coordinates(),button_settings.getsize())) #adding the seventh button to the button array
button_help = Button(screen,"Help",(177,177,177),100,SCREEN_WIDTH-80,20,(60,60)) #creating an object from the button class    
buttons.append((button_help.coordinates(),button_help.getsize())) #adding the seventh button to the button array
...
while run: #this is the game loop that'll run while the session is running 
    ...
    drawGraph(graph,shortest_path_array,canvas_colour,image,calculated_costs,stats)
    
    if programState == "link" and first_link != None:
        drawLinkLine(first_link[0][1],event.pos)
    ...
    if hovering_node != None:
        drawIndividualNode(hovering_node,(0,0, 255)) #an individual node will be drawn with a blue colour (RGB)
    ...
    button_move.draw(False,pygame.mouse.get_pos()) #calls the draw method from the button class
    button_delete.draw(False,pygame.mouse.get_pos())
    button_create.draw(False,pygame.mouse.get_pos())
    button_link.draw(False,pygame.mouse.get_pos())
    button_edit.draw_icon("images\\edit.ico") #draws the edit icon to the screen
    button_run.draw_icon("images\\media_play_green.ico") #draws the run icon to the screen
    button_settings.draw_icon("images\\gear.ico") #draws the settings icon to the screen
    button_help.draw_icon("images\\help2.ico") #draws the settings icon to the screen
    
    if programState == "run": #this will be true if the "run" button was the last to be pressed
        #checks that the graph has the right amount of starting/destination nodes
        if is_graph_valid(graph) == True: 
            ...
            if algorithm == "d": #checks to see if the dijkstra algorithm has been set
                ...
                #calculates the shortest path using the algorithm
                shortest_path_array, cost, calculated_costs = shortest_path.dijkstra() 
                ...
                start = time.perf_counter() #takes timestamp before the algorithm starts
                #calculates the shortest path using the algorithm
                shortest_path_array,cost,calculated_costs = shortest_path.aStar()
                end = time.perf_counter() #takes timestamp when the algorithm finishes
                elapsed = end - start #subtacts the start/end timestamps to find the elpsed time
        else:
    ...
    for event in pygame.event.get(): #handles the events 
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1: #detects if the left mouse button is pressed
            ...
            if temp != None: #this will be True if a button has been pressed 
                ...
                if button_pressed == 0: 
                    ...
                    settings_info = settingsmenu(algorithm,graph,canvas_colour,screen_title,image,stats)
                    ...
                    webbrowser.open("https://ollietucker.com/help.html")
                ...
                if programState == "create": #if the create button has been pressed 
                    ...
                    if inCanvas(event.pos[0],event.pos[1], radius) == True: #checks if the node is inside the canvas
                        if len(graph) < 50: #if there are less than 50 nodes, a node will create 
                            nodeID = nodeID + 1 #increments the node ID by one 
                            if graph == []: #checks if the graph is empty
                                nodeID = 1 #node ID is set to one if the graph is empty
                        ...
                        else: #this will run if there is more than 50 nodes 
                            show_error_message("You cannot create more that 50 nodes!") #an error mesage will show with htis text
        ...
        if event.type == pygame.MOUSEBUTTONUP and first_link != None and programState == "link":
            temp6 = detect_circle(event.pos,graph)
            if temp6 != None:
                second_link = temp6
        ...
        if programState == "link" and first_link != None and detect_circle(event.pos,graph)  != None and detect_circle(event.pos,graph)  != first_link:
            hovering_node = detect_circle(event.pos,graph) 
        else:
            hovering_node = None
